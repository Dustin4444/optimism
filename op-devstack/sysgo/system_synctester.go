...
func DefaultSimpleSystemWithSyncTester(dest *DefaultSimpleSystemWithSyncTesterIDs) stack.Option[*Orchestrator] {
	...
	opt.Add(WithSyncTesters(ids.SyncTester, []stack.L2ELNodeID{ids.L2EL}))

	// Example of configuring a CL node with a SyncTester instead of an L2EL node
	// This demonstrates the new functionality where WithL2CLNode can accept either
	// an L2EL node or a SyncTester. When using a SyncTester, the CL node connects
	// directly to the SyncTester endpoint without relying on an L2EL node.
	opt.Add(WithL2CLNodeWithSyncTester(ids.L2CL2, true, false, ids.L1CL, ids.L1EL, ids.SyncTester))
